...
@st.cache_resource(show_spinner='Conectando con el catálogo de productos')
...
@traceable(
    run_type="llm",
    name="aws_webinar_test",
    project_name="webinar",
    tags=["Claude3"],
    metadata={"ls_provider": ls_provider, "ls_model_name": modelId}
)
...
@traceable(
    ...
    project_name="webinar",
    tags=["embedding"],
    metadata={"ls_provider": ls_provider, "ls_model_name": embedding_modelId}
)
def log_embedding_model(messages: list, input_tokens: int, result: str):
    return {
        "response": [
            {
...
def get_claude_description(input_image):
    ...
    messages = json.dumps({
        ...
        "max_tokens": 1024,
        "temperature" : 0,
        "messages": get_claude_messages_content(input_image)
    })
...
def generate_titan_embedding(input_image, input_text):
...
def on_get_embedding():
    st.session_state.embedding = generate_titan_embedding(image_to_base64(), st.session_state.description)
    inputs = [
        HumanMessage(
            content = [
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{image_to_base64()}",
                    },
                },
                {"start_time": datetime.now(timezone.utc)},
                {
    ...
    log_embedding_model(inputs, 
                        st.session_state.embedding_tokens, 
...
#st.write(st.session_state.similar_vectors)
